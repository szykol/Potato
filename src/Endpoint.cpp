...
bool EndpointData::AllowsMethod(RequestMethod method) {
    if (method == RequestMethod::NONE) throw InvalidRequestMethod();
...
std::string EndpointData::EnumToStr(RequestMethod method) {
    ...
    if (method == RequestMethod::UPDATE) return "UPDATE";
    ...
    if (method == RequestMethod::CONNECT) return "CONNECT";
...
RequestMethod EndpointData::StrToEnum(const std::string& method)
{
    ...
    if (method == "UPDATE") return RequestMethod::UPDATE;
    ...
    if (method == "CONNECT") return RequestMethod::CONNECT;
